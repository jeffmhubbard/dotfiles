...
LAYOUT_PATH=$HOME/.cache/i3/layouts

# make directory for storing layouts
mkdir -p $LAYOUT_PATH > /dev/null 2>&1

# logs
LOG_FILE=$HOME/.cache/i3/layout_manager.log
...
Delete Layout" | rofi -i -dmenu -no-custom -p "Select action")
  ...
  LAYOUT_NAME=$(echo "$LAYOUT_NAMES" | rofi -i -dmenu -p "Select layout or type new name" | sed 's/\s/_/g') # ask for selection
...
Specific (Choose)
Match Any" | rofi -i -dmenu -p "How to identify windows?")
